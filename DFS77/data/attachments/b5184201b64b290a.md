...
- Name: navigation/footer-nav.spec.ts >> Bottom nav — tab routing & active indicator (DFS-1998) >> Feed tab routes to /challenges/feed and takes the indicator
- Location: tests/navigation/footer-nav.spec.ts:42:5
...
=========================== logs ===========================
waiting for navigation to "**/challenges/feed" until "load"
============================================================
```
...
```ts
  ...
  31  | 
  32  | test.describe('Bottom nav — tab routing & active indicator (DFS-1998)', { tag: ['@navigation', '@dfs1998'] }, () => {
...
> 52  |             await page.waitForURL('**/challenges/feed');
  ...
  103 |     });
  104 | 
  105 |     test('Only one tab is active at a time and Home restores', { tag: '@smoke' }, async ({ loggedInPage: page }) => {
