...
> 225 |                 expect(body?.shareUrl, 'share API should return shareUrl').toBeTruthy();
  ...
  264 |           await tPage.waitForURL((url) => HOME_PATH.test(url.pathname), { timeout: 60_000 });
  265 |           await expect(tContest.submissionReady).toBeVisible({ timeout: 30_000 });
  266 |         });
  267 | 
  268 |         await test.step(`Tailer's persistent slip holds the sharer's ${PICK_COUNT} pick ids`, async () => {
  269 |           // pickPlayers returns DOM ids like "player-1314", but pp_persistent_slip:v1's
  270 |           // selectedPicks is keyed by the bare player id ("1314") — strip the prefix.
  271 |           const expected = pickIds.map((id) => id.replace(/^player-/, '')).sort();
  272 |           await expect
  273 |             .poll(
  274 |               async () =>
  275 |                 tPage.evaluate((key) => {
  276 |                   const raw = localStorage.getItem(key);
  277 |                   if (!raw) return [];
  278 |                   try {
  279 |                     return Object.keys(JSON.parse(raw).selectedPicks ?? {}).sort();
  280 |                   } catch {
  281 |                     return [];
  282 |                   }
  283 |                 }, SLIP_STORAGE_KEY),
  284 |               {
  285 |                 timeout: 15_000,
  286 |                 message: "Tailer's persistent slip should match Sharer's pick IDs",
  287 |               },
  288 |             )
  289 |             .toEqual(expected);
  290 |         });
  291 | 
  292 |         await test.step(`Tailer places the tailed $${ENTRY_AMOUNT} entry — success page is shareable`, async () => {
  293 |           await tContest.setEntryAmountIfEditable(ENTRY_AMOUNT);
  294 |           await tContest.placePick();
  295 |           await expect(
  296 |             tPage.getByRole('button', { name: 'Continue' }).filter({ visible: true }).first(),
  297 |           ).toBeVisible();
  298 |           await tSuccess.assertContestShareable();
  299 |         });
  300 |       } finally {
  301 |         await sharerCtx?.close();
  302 |         await tailerCtx?.close();
  303 |       }
  304 |     });
  305 |   },
  306 | );
