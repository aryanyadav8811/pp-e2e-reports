...
- Name: navigation/legal.spec.ts >> Legal - contact >> /contact-us renders no HubSpot ticket form on any breakpoint
...
Call log:
  ...
  - waiting for locator('#hubspotForm')
...
> 99  |     await expect(contact.hubspotForm).toHaveCount(0);
  ...
  133 |         (paths) =>
  134 |           Promise.all(
  135 |             paths.map(async (path) => {
  136 |               const response = await fetch(path, { credentials: 'include' });
  137 |               return { path, status: response.status };
  138 |             }),
  139 |           ),
  140 |         internal,
  141 |       );
  142 |       for (const { path, status } of statuses) {
  143 |         expect.soft(status, `footer link ${path} resolves`).toBe(200);
  144 |       }
  145 |     });
  146 |   },
  147 | );
