...
- Name: auth/login.spec.ts >> Login - Page Rendering >> login page is accessible via direct URL navigation
- Location: tests/auth/login.spec.ts:40:3
...
Locator: locator('#username').filter({ visible: true }).first()
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('#username').filter({ visible: true }).first()
